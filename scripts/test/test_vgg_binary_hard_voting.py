#!/usr/bin/env python
"""
Test pretrained VGG Binary Sequence Classifier

This script:
  - Downloads test data from Azure Blob Storage.
  - Loads a pretrained VGG model (.pth file).
  - Runs evaluation on the test dataset using sequence-level chunked inference.
...
def test_model(model, test_loader, criterion, device, outputs_dir):
    my_logger.info("Starting testing phase...")
    model.eval()
    test_loss = 0.0
    test_correct = 0
    test_total = 0
    ...
    with torch.no_grad():
        for seq_slices, seq_labels in test_loader:
            ...
            slice_logits = torch.cat(logits_chunks, dim=1)
            seq_logits = slice_logits.mean(dim=1)
    ...
    my_logger.info(
        ...
        f"Recall={test_recall:.2f}, Precision={test_precision:.2f}, F1={test_f1:.2f}, AUC={test_auc:.2f}"
    )
    mlflow.log_metrics({
        "test_loss": test_loss,
...
def main():
    parser = argparse.ArgumentParser(description="Test pretrained VGG binary sequence model")
