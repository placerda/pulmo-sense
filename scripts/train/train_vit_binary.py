#!/usr/bin/env python
"""
Train Vision Transformer binary model with sequence-level validation.
...
Validation is done sequence-level in small chunks to reduce peak GPU memory usage.
"""
...
def train_model(train_loader, val_seq_loader, num_epochs, learning_rate, model_name, chunk_size=10):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, (inputs, _, labels) in enumerate(train_loader):
            ...
            if i % 50 == 0:
                ...
                my_logger.info(f"[Train] Batch {i}/{len(train_loader)} — "
        ...
        # — Validation (sequence-level, chunked) —
        model.eval()
        ...
        with torch.no_grad():
            for seq, labels in val_seq_loader:
                ...
                for start in range(0, L, chunk_size):
                    ...
                    chunk = seq[:, start:end]  # [B, chunk_len, C, H, W]
                    Bc, Lc, _, _, _ = chunk.shape
                    flat = chunk.reshape(Bc * Lc, C, H, W)
                    flat = flat.repeat(1, 3, 1, 1)
                    flat = nn.functional.interpolate(flat, size=(224, 224),
                                                     mode='bilinear', align_corners=False)
                    out = model(flat)  # [Bc*Lc, 2]
                    logits_chunks.append(out.view(Bc, Lc, -1))
        ...
        val_recall    = recall_score(y_true, y_pred, average='binary', pos_label=1, zero_division=0)
        val_precision = precision_score(y_true, y_pred, average='binary', pos_label=1, zero_division=0)
        val_f1        = f1_score(y_true, y_pred, average='binary', pos_label=1, zero_division=0)
        try:
        ...
        if val_recall > best_recall:
            ...
            best_epoch  = epoch
            best_metrics.update({'loss': val_loss,
                                 'acc': val_acc,
                                 'precision': val_precision,
                                 'f1': val_f1,
                                 'auc': val_auc})

            prefix = f"vit_{model_name}_{epoch}ep_{learning_rate:.5f}lr_{val_recall:.3f}rec"
    ...
    # — Log all best‐model metrics to MLflow (using best_epoch as step) —
    final_metrics = {
        "best_model_epoch": best_epoch,
        "best_model_loss":  best_metrics['loss'],
        "best_model_acc":   best_metrics['acc'],
        "best_model_recall":best_metrics['precision'],
...
def main():
    ...
    parser.add_argument("--chunk_size",      type=int, default=10, help="Number of slices per forward chunk during validation")
    ...
    val_seq_dataset = DatasetSequence2DBinary(
        dataset_folder=args.val_dir,
        sequence_length=args.sequence_length
    )

    train_loader   = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    val_seq_loader = DataLoader(val_seq_dataset, batch_size=args.val_batch_size, shuffle=False)
    my_logger.info(
        f"Loaded {len(train_dataset)} training samples and "
        f"{len(val_seq_dataset)} validation sequences."
    )
